...
attractions_spec = pd.read_csv("AttractionsSpec.csv")
attractions_dist = pd.read_csv("AttractionsDist.csv")
...
def get_information():
    ...
    while True:
        usr_position = input("Which is the nearest attraction you can see? ").strip().lower().replace(" ","")
        if usr_position in attractions_spec["attraction"].strip().lower().replace(" ",""):
            break
        else:
            print("Attraction name not found, please digit it again.")
    print("Perfect!")    
    while True:
        usr_flashpass = input("Do you have a flashpass? Reply \"yes\" or \"no\")").strip().lower().replace(" ","")
    ...
    while True:
        usr_age = input("How old are you? Write it in number.\n (If you are in a group consider the age of the youngest person)").strip().replace(" ","")
        try:
            int(usr_age)
            break
        except:
    ...
    while True:
        usr_water = input("Do you want to do an attraction with the chance of getting wet or not? Reply \"yes\" or \"no\")").strip().lower().replace(" ","")
